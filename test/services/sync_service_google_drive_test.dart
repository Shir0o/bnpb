...
void main() {
  ...
  test('downloads every unprocessed remote delta file from mobile devices',
      () async {
    final remoteFiles = [
      drive.File()
        ..id = 'phone-1000'
    ...
    when(() => googleDrive.downloadFile(any(), any())).thenAnswer((invocation) {
  ...
  test('manual Google Drive sync reports a setup error when signed out',
      () async {
  ...
  test('local sync status reports a missing folder before manual sync',
      () async {
