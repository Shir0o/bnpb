...
void main() {
  group('ModelManager.ensureFreeSpace', () {
    ...
    test(
      'throws InsufficientStorageException when probe reports too little',
      () async {
        final manager = ModelManager(
          requiredFreeBytes: 1000,
    ...
    test(
      'passes (no-op) when probe returns null (unknown free space)',
      () async {
        final manager = ModelManager(
          requiredFreeBytes: 1000,
    ...
    test(
      'InsufficientStorageException.toString surfaces a friendly message',
      () {
