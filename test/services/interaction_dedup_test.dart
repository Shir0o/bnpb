...
void main() {
  ...
  test(
    'InteractionDao.findDuplicateInteractions detects duplicates without modifying data',
    () async {
      final db = await databaseFactory.openDatabase(inMemoryDatabasePath);
      final dbHelper = DBHelper();
      DBHelper.setDatabaseForTest(db);
      ...
      // Run findDuplicateInteractions (dry run)
      final dupGroups = await dao.findDuplicateInteractions();
      expect(dupGroups.length, 1);
      expect(dupGroups.first.primary.id, intId1);
      expect(dupGroups.first.duplicates.length, 1);
      expect(dupGroups.first.duplicates.first.id, intId2);

      // Verify database is completely untouched (Duplicate 2 is NOT soft-deleted)
      final dupRows = await db.query(
        'interactions',
        where: 'id = ?',
        whereArgs: [intId2],
      );
      expect(dupRows.first['deletedAt'], isNull);
