...
void main() {
  group('RelationshipInsightsRepository', () {
    ...
    test(
      'flags a drifting contact when the current gap dwarfs the median',
      () async {
        db.contacts.add(
          Contact(
            id: 'c1',
    ...
    test(
      'does not flag drift when current gap matches typical cadence',
      () async {
        db.contacts.add(
          Contact(
            id: 'c1',
    ...
    test('flags a silence streak past the 60-day threshold', () async {
      db.contacts.add(
        Contact(
          id: 'c1',
          firstName: 'Bob',
          interactions: [interaction(now.subtract(const Duration(days: 75)))],
        ),
      );
    ...
    test(
      'skips silence streak when the contact was contacted recently',
      () async {
        db.contacts.add(
          Contact(
            id: 'c1',
            firstName: 'Bob',
            interactions: [interaction(now.subtract(const Duration(days: 10)))],
          ),
        );
    ...
    test(
      'flags stale prayer requests when 3+ pending are older than 30 days',
      () async {
        db.contacts.add(
          Contact(
            id: 'c1',
    ...
    test(
      'computes the follow-up completion rate when sample is large enough',
      () async {
